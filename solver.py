...
def integrate_in_s_0(matrix, y_0):
    ...
    for i in range(rows):
        for j in range(cols):
            out[i][j] = y_0.integrate_in_cube(matrix[i][j], [x_, y_, t_], [0, 0, -10], [1, 1, 0], 3)
    return out
...
def integrate_in_s_gamma(matrix, y_gamma):
    ...
    for i in range(rows):
        for j in range(cols):
            out[i][j] = y_gamma.integrate_in_cube(matrix[i][j], [x_, y_, t_], [-10, -10, 0], [10, 10, 1], 3) \
                      - y_gamma.integrate_in_cube(matrix[i][j], [x_, y_, t_], [0, 0, 0], [1, 1, 1], 2)
    return out
